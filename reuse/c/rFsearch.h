 ...
 * Id: rFsearch.h,v 1.39 2009/08/03 15:25:47 vollmer Exp $
 */
...
# define rFSearchVersion "2009-08-03"
 ...
 *
 * rFindFile, rFindFileSet, rFsearchUsedSearchPath
 * ---------------------------------------------------
 ...
 *       IF   rFileIsReadable (D/NE)
 *       THEN store D so that next call of rFsearchUsedSearchPath()
 *                  returns D
 *            RETURN D/NE
 *       FI
 *     END;
...
extern char *rFsearchUsedSearchPath(void);
/* Return the search path which is used by a previous call of rFindFile/rFindFileSet
 * to locate the search file.
 * - The value is valied only, if rFindFile/rFindFileSet found the file, otherwiese
 *   the returned value is undefined.
 * - The returned value may be NULL, if no search path has been used (e.g. an absolute path
 *   was given to rFindFile/rFindFileSet.
 */
 ...
 * Note for #include_next:
 *	`path' must be the search path used to locate the file containing
 *      the file containing the #include_next directive
 */
...
/*
 * Log: rFsearch.h,v $
 * Revision 1.39  2009/08/03 15:25:47  vollmer
 * added rFsearchUsedSearchPath()
 * fixed rSetSearchPathStart()
 *
