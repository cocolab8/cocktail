 ...
 * Id: rFsearch.c,v 1.56 2007/10/08 15:29:55 vollmer Exp $
 */
...
# if _MSC_VER >= 1400
  /* MS Visual Studio 2005 changed names (and returned values) ... */
  # include <io.h>
...
typedef struct sDirEntry {
  ...
  long           Mark;    /* Mark >= 0  : given by the user
			   ...
			   * are appended at the the end of the directory list.
			   */
 ...
 */
{
  ...
  if (!dir->Exists) {
    /* directory does not exists in filesystem */
    DEXIT (("read_dir: does not exists in filesystem\n"));
    return rfalse;
  }
...
char *rSetSearchPathStart (const char *path)
{
  ...
  if (path == NULL) {
    ...
    for (dir = get_search_path(); dir != NULL; dir = dir->Next) {
      char *s = strstr (dir->Dir.Str, rSetSearchPathStart_ls.Str);
      if (s == dir->Dir.Str) {
	/* found a substring */
	dir = dir->Next;
	break;
